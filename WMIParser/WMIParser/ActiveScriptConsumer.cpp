...
const wchar_t ActiveScriptConsumerClass::CONSUMER_NAME[] = L"ActiveScriptEventConsumer";

const ConsumerDataType ActiveScriptConsumerClass::ConsumerDataTypes[ConsumerDataTypesSize] = { // the order on disk
    { TS_STRING,    4, L"MachineName" },
    { TS_UINT32,    4, L"MaximumQueueSize" },
    { TS_BYTEARRAY, 4, L"CreatorSID" },
    { TS_STRING,    4, L"Name" },
    { TS_STRING,    4, L"ScriptingEngine" },
    { TS_STRING,    4, L"ScriptText" },
    { TS_STRING,    4, L"ScriptFilename" },
    { TS_UINT32,    4, L"KillTimeout" }
};


void ActiveScriptConsumerClass::SetName(uint64 s, uint64 c, int type) {
  Name.Set(s, c, type);
}

void ActiveScriptConsumerClass::SetScriptingEngine(uint64 s, uint64 c, int type) {
  ScriptingEngine.Set(s, c, type);
}

void ActiveScriptConsumerClass::SetScriptText(uint64 s, uint64 c, int type) {
  ScriptText.Set(s, c, type);
}

void ActiveScriptConsumerClass::SetScriptFilename(uint64 s, uint64 c, int type) {
  ScriptFilename.Set(s, c, type);
}

void ActiveScriptConsumerClass::SetKillTimeout(uint32 val) {
  KillTimeout = val;
}

void ActiveScriptConsumerClass::SetName(std::vector<ExtentClass>& extents, int type) {
  if (extents.size() == 1)
...
void ActiveScriptConsumerClass::SetScriptingEngine(std::vector<ExtentClass>& extents, int type) {
  if (extents.size() == 1)
    ScriptingEngine.Set(extents.at(0).GetStart(), extents.at(0).GetCount(), type);
  else
    ScriptingEngine.Set(extents, type);
}

void ActiveScriptConsumerClass::SetScriptText(std::vector<ExtentClass>& extents, int type) {
  if (extents.size() == 1)
    ScriptText.Set(extents.at(0).GetStart(), extents.at(0).GetCount(), type);
  else
    ScriptText.Set(extents, type);
}

void ActiveScriptConsumerClass::SetScriptFilename(std::vector<ExtentClass>& extents, int type) {
  if (extents.size() == 1)
    ScriptFilename.Set(extents.at(0).GetStart(), extents.at(0).GetCount(), type);
  else
    ScriptFilename.Set(extents, type);
}

void ActiveScriptConsumerClass::Print(HANDLE hFile, FILE *out) {
  ...
  MyPrintFunc(out, L"ScriptingEngine: ");
  ScriptingEngine.Print(hFile, out);
  MyPrintFunc(out, L"ScriptFilename: ");
  ScriptFilename.Print(hFile, out);
  MyPrintFunc(out, L"ScriptText: ");
  ScriptText.Print(hFile, out);
  MyPrintFunc(out, L"\r\n=============================================================================\r\n");
}

bool ActiveScriptConsumerClass::IsConsumer(const void* recordBuf, uint32 size, bool bXP) {
...
EventConsumer* ActiveScriptConsumerClass::Create(const void* recordBuf, std::vector<ExtentClass>& cRecordExtents, uint32 size, bool bXP) {
  if (recordBuf && cRecordExtents.size() && size) {
    ActiveScriptConsumerClass * pObject = new ActiveScriptConsumerClass;
    if (pObject) {
      wchar_t wszGUIDStr[MAX_STRING_WIN7_COUNT + 1];
      if (bXP)
        MD5Hash::GetStr(CONSUMER_NAME, wszGUIDStr, _countof(wszGUIDStr));
                            ...
                            if (dataoffset + sizeof(byte) < size) { // first byte of a string starts with a 0 byte
                              if (!*(parseBuf + dataoffset)) {
                                ...
                                if (indataoffset + indatasize < datasize) {
                                  ...
                                  else if (!_wcsicmp(ConsumerDataTypes[i].ColumnName, L"ScriptingEngine"))
                                    pObject->SetScriptingEngine(ex, TS_STRING);
                                  else if (!_wcsicmp(ConsumerDataTypes[i].ColumnName, L"ScriptText"))
                                    pObject->SetScriptText(ex, TS_STRING);
                                  else if (!_wcsicmp(ConsumerDataTypes[i].ColumnName, L"ScriptFilename"))
                                    pObject->SetScriptFilename(ex, TS_STRING);
                                  else
                                    goto Exit;
                                }
                                else
